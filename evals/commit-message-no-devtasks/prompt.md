...
1. Create a temporary directory with `git init`, a `.gitignore` containing
   `.claude/dev/`, an initial commit, and a `feature/retry` branch
   checked out.
...
4. Then: **commit this** — both edits belong to the same task.
